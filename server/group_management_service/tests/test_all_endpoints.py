...
group_id = None  # To be set after creation
...
try:
    ...
    if response.status_code not in [200, 201]:
        raise ValueError(f"Creation failed: {response.text}")
    ...
    print(f"❌ FAIL: {e}")
    print("Skipping subsequent tests that require group_id")
    group_id = None  # Prevent further tests
...
# Test 7: GET /groups with pagination (independent)
